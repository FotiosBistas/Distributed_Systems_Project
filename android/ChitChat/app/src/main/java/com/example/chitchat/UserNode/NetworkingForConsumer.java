...
public class NetworkingForConsumer implements Runnable{
    ...
    public NetworkingForConsumer(Socket request_socket,UserNode cons,int operation){
        ...
        try {
            ...
            System.out.println(ConsoleColors.RED + "Socket Error while constructing networking for consumer" + ConsoleColors.RESET);
            shutdownConnection();
            return;
        }catch (IOException e) {
            System.out.println(ConsoleColors.RED + "Error while constructing networking for consumer" + ConsoleColors.RESET);
            shutdownConnection();
            return;
        }
    }
