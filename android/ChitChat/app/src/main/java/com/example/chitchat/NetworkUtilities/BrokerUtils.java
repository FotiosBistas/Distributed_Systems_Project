...
import com.example.chitchat.Broker.*;
...
public class BrokerUtils {
    ...
    public static Integer ServeRegisterRequest(ObjectInputStream localinputStream,ObjectOutputStream localoutputStream,Socket socket,Broker broker){
        ...
        if(correct) {
            ...
            if(topic == null){
                System.out.println(ConsoleColors.RED + "There is no such topic in the topic list of the broker" + ConsoleColors.RESET);
                if(GeneralUtils.sendMessage(Messages.NO_SUCH_TOPIC,localoutputStream) == null){
                    return null;
                }
            ...
            System.out.println(ConsoleColors.RED + "This is not the correct broker for the topic" + ConsoleColors.RESET);
            return null;
        }
        return -1;
    }
    ...
    public static String receiveNickname(ObjectInputStream localinputStream, Socket socket) {
        //System.out.println( ConsoleColors.GREEN + "Receiving client's nickname" + ConsoleColors.RESET);
        String nickname;
        if((nickname = SHA1.encrypt(GeneralUtils.readUTFString(localinputStream,socket))) == null){
            return null;
        }
        //System.out.println(ConsoleColors.GREEN + "Client's nickname is: " + nickname + ConsoleColors.RESET);
        return nickname;
    }
