...
public class  Broker{
    ...
    //before assigning numbers to pool threads. Runtime.getRuntime().availableProcessors()
    // in my local machine I have 12
    ...
    private static final ExecutorService brokerServices = Executors.newFixedThreadPool(3);

    private static final ExecutorService publisher_event_loop = Executors.newSingleThreadExecutor();

    private static final ExecutorService consumer_event_loop = Executors.newSingleThreadExecutor();

    private static final ExecutorService broker_event_loop = Executors.newSingleThreadExecutor();

    private static final ExecutorService interBrokerComms = Executors.newFixedThreadPool(1);
    ...
    public void readBrokerListFromConfigFile(){
        try {
            ...
            while((line = bufferedReader.readLine()) != null) {
                ...
                System.out.println("The ports are: " + Arrays.toString(ports));
    ...
    public void startBroker() {
        try {
            ...
            //separate thread for receiving consumer and broker connections
            consumer_event_loop.submit(new Thread(() -> {
                try {
                    consumer_service = new ServerSocket(consumer_port);
                    System.out.println("Opened thread to receive consumer connections");
            ...
            publisher_event_loop.submit(new Thread(() -> {
                try {
            ...
            /*broker_event_loop.submit(new Thread(() -> {
                try {
    ...
    private void notifyBrokersOnChanges(Object object, String topic_name, SendObject.Operation operation){
        try {
            for (Tuple<String, int[]> broker : BrokerList) {
                if (broker.getValue1().equals(this.ip) && broker.getValue2()[0] == this.consumer_port && broker.getValue2()[1] == this.publisher_port) {
                    System.out.println("Same broker");
                } else {
                    ...
                    interBrokerComms.submit(thread);
                }
            }
        }catch (IOException ioException){
            System.out.println(ConsoleColors.RED + "Error on notifying brokers on changes" + ConsoleColors.RESET);
        }
    ...
    public  void addConsumerToTopic(Topic topic, String consumer){
        ...
        if (Topics.contains(topic)) {
            ...
            interBrokerComms.submit(new Thread(()->{notifyBrokersOnChanges(consumer,topic.getName(),SendObject.Operation.SHARE_SUBSCRIBER);}));
    ...
    public void UnsubscribeFromTopic(Topic topic, String consumer){
        if(Topics.contains(topic)){
            ...
            interBrokerComms.submit(new Thread(()->{notifyBrokersOnChanges(consumer,topic.getName(),SendObject.Operation.SHARE_DISCONNECT);}));
        }
        System.out.println("Subscribed users: ");
        System.out.println(Topics.get(Topics.indexOf(topic)).getSubscribedUsers());
    }
     ...
     * @param topic_name
     */
    public synchronized void addSubcriberFromOther(String value,int id,String topic_name) {
     ...
     * @param topic_name
     */
    public synchronized void DisconnectFromOtherTopic(String value,int id,String topic_name){
        Topic right_topic = null;
        ArrayList<Topic> Other_Broker_Topics = Topics_From_Other_Brokers.get(id);
        for (Topic topic:Other_Broker_Topics) {
            if(topic.getName().equals(topic_name)){
     ...
     * @param topic_name
     */
    public synchronized void addMessageFromOtherBroker(Value value,int id,String topic_name) {
        ...
        if(value instanceof Story){
            right_topic.addToStoryQueue((Story)value);
        }else if(value instanceof MultimediaFile){
            right_topic.addToFileQueue((MultimediaFile) value);
        }else if(value instanceof Text_Message){
            right_topic.addToMessageQueue((Text_Message) value);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        if(args.length <= number_of_ports) {
